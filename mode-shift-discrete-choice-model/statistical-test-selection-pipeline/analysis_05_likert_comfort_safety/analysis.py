#!/usr/bin/env python3
"""
analysis.py — Physical Comfort Statistical Test Selection Domain
======================================================
...
outcome  = [c for c in df.columns if "physical comfort" in c.lower()][0] if "physical comfort" in df.columns or any("physical comfort" in c.lower() for c in df.columns) else "How would you rate the physical comfort (Seating space, AC, Noise) of your current mode?"
...
print(f"=== PHYSICAL COMFORT ANALYSIS ({len(res_df)} groupings) ===")
if len(res_df) > 0:
